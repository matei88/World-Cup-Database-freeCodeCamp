...
echo "$($PSQL "SELECT g.year, t.name FROM public.teams t JOIN public.games g on t.team_id = g.winner_id WHERE team_id IN (SELECT winner_id FROM public.games WHERE round = 'Final') AND round = 'Final' ORDER BY year")"
